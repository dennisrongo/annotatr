...
.DESCRIPTION
  ...
  an env var to "" deletes it). Set TAURI_SIGNING_PRIVATE_KEY_PASSWORD in
  .env.release to the key's password ("" if it has none).
...
function Invoke-Preflight {
  ...
  if ([string]::IsNullOrWhiteSpace($key)) {
    Add-Check 'Updater key' 'FAIL' 'TAURI_SIGNING_PRIVATE_KEY not set' 'Copy ~/.tauri/annotatr-updater.key from the Mac, then set TAURI_SIGNING_PRIVATE_KEY in .env.release (path or base64 contents).'
  ...
  if (-not [string]::IsNullOrWhiteSpace($key) -and (Has-Command 'npx')) {
    ...
    try {
      ...
      if ($LASTEXITCODE -eq 0) {
        ...
        Add-Check 'Updater key can sign' 'FAIL' $hint 'Set TAURI_SIGNING_PRIVATE_KEY_PASSWORD in .env.release to the key''s password (use "" if the key has none).'
      }
    } finally {
  ...
  if (-not (Has-Command 'git')) {
    ...
    if ($LASTEXITCODE -ne 0) {
      Add-Check 'Git repo' 'FAIL' '' 'Run this from inside the annotatr clone.'
    } else {
...
# --- Locate repo root + load .env.release ------------------------------------
...
if (-not $Repo) { $Repo = 'dennisrongo/annotatr' }
